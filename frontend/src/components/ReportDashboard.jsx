...
// import React, { useState, useRef } from "react";
// import { UploadCloud, FileType, AlertCircle, Loader2, Download, BarChart3, FileText, BrainCircuit } from "lucide-react";
// import axios from "axios";
...
//     try {
//       const response = await axios.post("http://localhost:8000/api/generate-report", formData, {
//         headers: { "Content-Type": "multipart/form-data" },
//       });
//       setReportData(response.data);
//     } catch (err) {
//       setError(err.response?.data?.detail || "An error occurred while generating the report. Make sure backend is running and GEMINI_API_KEY is set.");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//   const handleDownloadPDF = () => {
//     if (!reportData || !reportData.pdf_base64) return;
//     const linkSource = `data:application/pdf;base64,${reportData.pdf_base64}`;
//     const downloadLink = document.createElement("a");
//     const fileName = `Analysis_Report_${file?.name.split('.')[0]}.pdf`;
//     downloadLink.href = linkSource;
//     downloadLink.download = fileName;
//     downloadLink.click();
//   };
...
//           <div
//             className={`glass rounded-3xl p-10 text-center cursor-pointer transition-all duration-300 border-2 border-dashed ${
//               isDragging ? "border-emerald-500 bg-emerald-500/5 scale-[1.02]" : "border-gray-700 hover:border-emerald-400/50 hover:bg-white/[0.02]"
//             }`}
//             onDragOver={handleDragOver}
//             onDragLeave={handleDragLeave}
//             onDrop={handleDrop}
//             onClick={() => fileInputRef.current?.click()}
//           >
//             <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileChange} accept=".csv, .xls, .xlsx" />

//             <div className="w-20 h-20 mx-auto bg-gray-800 rounded-full flex items-center justify-center mb-6 shadow-inner">
//               <UploadCloud className={`w-10 h-10 ${isDragging ? "text-emerald-400" : "text-gray-400"}`} />
//             </div>
...
//             </h3>
//             <p className="text-gray-400 mb-6">
//               {file ? `Ready to analyze ${(file.size / 1024).toFixed(2)} KB` : "Supports CSV, XLS, XLSX up to 50MB"}
//             </p>
...
//                   placeholder="Optional: Enter Gemini API Key"
//                   value={apiKey}
//                   onChange={(e) => { e.stopPropagation(); setApiKey(e.target.value); }}
//                   onClick={(e) => e.stopPropagation()}
//                   className="w-full bg-gray-900/50 border border-gray-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all text-center placeholder-gray-500"
//                 />
//                 <button
//                   onClick={(e) => { e.stopPropagation(); handleGenerateReport(); }}
//                   className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-bold py-3 px-8 rounded-xl transition-all shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:shadow-[0_0_30px_rgba(16,185,129,0.6)] flex items-center justify-center gap-2"
//                 >
//                   <BrainCircuit className="w-5 h-5" />
//                   Generate AI Report
//                 </button>
...
//             <div className="mt-8 text-center space-y-4">
//               <Loader2 className="w-10 h-10 animate-spin text-emerald-500 mx-auto" />
//               <p className="text-gray-400 animate-pulse">Analyzing data, generating charts, and writing insights...</p>
//             </div>
//           )}
//         </div>
//       )}
...
//               </h2>
//               <p className="text-gray-400 mt-1">
//                 {reportData.summary.num_rows} rows • {reportData.summary.num_cols} columns
//               </p>
//             </div>
//             <div className="flex gap-4">
//               <button
...
//               <button
//                 onClick={handleDownloadPDF}
//                 className="bg-emerald-500 hover:bg-emerald-400 text-black font-bold py-2 px-6 rounded-full transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)] flex items-center gap-2"
//               >
//                 <Download className="w-4 h-4" />
//                 Download PDF
//               </button>
...
//                   <BrainCircuit className="w-6 h-6 text-emerald-400" />
//                 </div>
//                 <h3 className="text-xl font-bold text-white">Executive Summary (AI Generated)</h3>
//               </div>

//               <div
//                 className="markdown-body text-gray-300"
//                 dangerouslySetInnerHTML={renderMarkdown(reportData.ai_insights)}
//               />
//             </div>
...
//                     <BarChart3 className="w-5 h-5 text-blue-400" />
//                   </div>
//                   <h3 className="text-lg font-bold text-white">Data Visualizations</h3>
//                 </div>

//                 <div className="space-y-6">
//                   {Object.entries(reportData.charts).map(([key, value]) => {
//                     const b64 = value;
//                     return (
//                     <div key={key} className="bg-gray-900/50 rounded-xl overflow-hidden border border-gray-800 hover:border-gray-700 transition-colors">
//                       <div className="p-3 bg-gray-800/50 border-b border-gray-800 text-sm font-medium capitalize text-gray-300">
//                         {key.replace('_', ' ')}
//                       </div>
//                       <img
...
//                   )})}
//                   {Object.keys(reportData.charts).length === 0 && (
//                     <p className="text-gray-500 text-center py-8">Not enough numerical data to generate charts.</p>
//                   )}
//                 </div>
//               </div>
//             </div>
